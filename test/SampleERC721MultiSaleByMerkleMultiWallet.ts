...
import { createTreeMultiWallet } from "../utils/merkletree"
...
import { SampleERC721MultiSaleByMerkleMultiWallet, SampleNFT } from "../typechain-types";
...
describe("Merkletree", function () {
  ...
  describe("exchange", () => {
    ...
    it("can not exchange: invalid proof", async function () {
      ...
      const tree1 = createTreeMultiWallet([{ userId: 1, address: authorized.address, allowedAmount: 1 }])

      await claim(tree1, sampleERC721MultiSaleByMerkleMultiWallet, owner, authorized, sampleNFT);
    ...
    it("can not exchange: over AL", async function () {
      ...
      const proof2 = tree2.getHexProof(ethers.utils.solidityKeccak256(['uint256','address', 'uint256'], [1, authorized.address, 5]))

      await expect(sampleERC721MultiSaleByMerkleMultiWallet.connect(authorized).exchange(1, [0, 1, 2, 3, 4], 5, proof2, { value: ethers.utils.parseEther("0.005") }))
        .not.to.be.reverted
      await expect(sampleERC721MultiSaleByMerkleMultiWallet.connect(authorized).exchange(1, [6], 5, proof2, { value: ethers.utils.parseEther("0.005") }))
        .to.be.revertedWith("claim is over allowed amount.")
    })
    ...
    it("can not exchange: over max supplay on sale", async function () {
      ...
      await expect(sampleERC721MultiSaleByMerkleMultiWallet.connect(authorized).exchange(1, [0, 1, 2, 3, 4], 10, proof, { value: ethers.utils.parseEther("0.005") }))
        .not.to.be.reverted
      await expect(sampleERC721MultiSaleByMerkleMultiWallet.connect(authorized).exchange(1, [5], 10, proof, { value: ethers.utils.parseEther("0.005") }))
        .to.be.revertedWith("claim is over the max sale supply.")
    })
  })
  ...
  describe("counting functions", () => {
    it("getBuyCount", async function () {
      ...
      expect(await sampleERC721MultiSaleByMerkleMultiWallet.connect(authorized).getBuyCount(1)).to.equals(5)
    ...
    it("reset next sale", async function () {
      ...
      expect(await sampleERC721MultiSaleByMerkleMultiWallet.connect(authorized).getBuyCount(1)).to.equals(0)
    })
  })
})
